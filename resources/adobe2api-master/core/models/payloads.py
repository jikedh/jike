...
def build_image_payload_candidates(
    *,
    ...
    if str(upstream_model_id or "").strip().lower() == "gpt-image":
        ...
        image_ids = [str(img_id) for img_id in source_image_ids[:3] if str(img_id)]
        ...
        candidates.append(web_edit_payload)

        official_references = [
            {"source": {"uploadId": img_id}, "usage": "general"}
            for img_id in image_ids
        ]
        asset_source_references = [
            {"source": {"uploadId": img_id}, "usage": "asset"} for img_id in image_ids
        ]
        id_general_references = [
            {"id": img_id, "usage": "general"} for img_id in image_ids
        ]
        id_asset_references = [
            {"id": img_id, "usage": "asset"} for img_id in image_ids
        ]

        for references, module in (
            (official_references, "text2image"),
            (official_references, "image2image"),
            (asset_source_references, "image2image"),
            (id_general_references, "image2image"),
            (id_asset_references, "image2image"),
        ):
            edited = copy.deepcopy(base_payload)
            edited["generationMetadata"] = {
                "module": module,
                "submodule": "ff-image-generate",
            }
            edited["referenceBlobs"] = references
            candidates.append(edited)

        for reference_key in ("referenceImages", "references"):
            edited = copy.deepcopy(base_payload)
            edited["generationMetadata"] = {
                "module": "image2image",
                "submodule": "ff-image-generate",
            ...
            edited[reference_key] = official_references
            candidates.append(edited)
        ...
        edited["referenceBlobs"] = []
        edited["modelSpecificPayload"]["referenceImages"] = official_references
        candidates.append(edited)
