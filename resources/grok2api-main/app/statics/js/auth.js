...
const adminKey = (() => {
  const store = _keyStore('grok2api_admin_key');
  return {
    get: async () => (await store.get()) || 'local-admin',
    set: store.set,
    clear: store.clear,
  };
})();
...
function adminLogout() { adminKey.clear(); webuiKey.clear(); location.href='/admin/account'; }
