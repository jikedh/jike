...
_NESTED_ENV_KEYS = {
    "PROXY_EGRESS_MODE": "proxy.egress.mode",
    "PROXY_EGRESS_PROXY_URL": "proxy.egress.proxy_url",
    "PROXY_EGRESS_RESOURCE_PROXY_URL": "proxy.egress.resource_proxy_url",
    "PROXY_EGRESS_SKIP_SSL_VERIFY": "proxy.egress.skip_ssl_verify",
}
...
def load_config(
    ...
    for env_key, env_val in os.environ.items():
        if not env_key.startswith(env_prefix):
            continue
        raw_key = env_key[prefix_len:].upper()
        nested_key = _NESTED_ENV_KEYS.get(raw_key)
        if nested_key:
            _set_nested(data, nested_key, env_val)
            continue
        parts = raw_key.lower().split("_", 1)
        if len(parts) == 2:
            section, key = parts
