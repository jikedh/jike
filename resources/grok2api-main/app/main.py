...
def create_app() -> FastAPI:
    ...
    if os.getenv("ADOBE2API_ENABLED", "true").strip().lower() not in {
        "0",
        "false",
        "no",
        "off",
    }:
        from app.integrations.adobe2api import load_adobe2api_app

        adobe_mount_path = os.getenv("ADOBE2API_MOUNT_PATH", "/adobe").strip()
        if not adobe_mount_path.startswith("/"):
            adobe_mount_path = f"/{adobe_mount_path}"
        adobe_mount_path = adobe_mount_path.rstrip("/") or "/adobe"
        app.mount(adobe_mount_path, load_adobe2api_app(), name="adobe2api")
