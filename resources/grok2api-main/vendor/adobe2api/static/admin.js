document.addEventListener("DOMContentLoaded", async () => {
  ...
  window.fetch = async (...args) => {
    ...
    if (res.status === 401) {
      window.location.href = "/adobe/login";
    }
    return res;
  };
  ...
  async function ensureAuthenticated() {
    try {
      const res = await rawFetch("/adobe/api/v1/auth/me", { method: "GET" });
      if (!res.ok) {
        window.location.href = "/adobe/login";
        return false;
      }
  ...
  async function loadTokens() {
    try {
      const res = await fetch("/adobe/api/v1/tokens");
  ...
  addBtn.addEventListener("click", async () => {
    ...
    try {
      const endpoint = tokens.length > 1 ? "/adobe/api/v1/tokens/batch" : "/adobe/api/v1/tokens";
  ...
  window.deleteToken = async (id) => {
    ...
    try {
      const res = await fetch(`/adobe/api/v1/tokens/${id}`, { method: "DELETE" });
      if (!res.ok) {
  ...
  window.toggleToken = async (id, newStatus) => {
    try {
      const res = await fetch(`/adobe/api/v1/tokens/${id}/status?status=${newStatus}`, { method: "PUT" });
      if (!res.ok) {
  ...
  window.refreshToken = async (id) => {
    ...
    try {
      const res = await fetch(`/adobe/api/v1/tokens/${id}/refresh`, { method: "POST" });
      if (!res.ok) {
        let detail = "刷新失败";
        try {
          const body = await res.json();
  ...
  window.refreshTokenCredits = async (id) => {
    ...
    try {
      const res = await fetch(`/adobe/api/v1/tokens/${id}/credits/refresh`, { method: "POST" });
      if (!res.ok) {
        let detail = "刷新积分失败";
        try {
          const body = await res.json();
  ...
  window.toggleAutoRefresh = async (id, enabled) => {
    try {
      const res = await fetch(`/adobe/api/v1/tokens/${id}/auto-refresh?enabled=${enabled ? "true" : "false"}`, {
        method: "PUT"
      });
      if (!res.ok) {
        let detail = "自动刷新设置失败";
  ...
  if (refreshCreditsBatchBtn) {
    refreshCreditsBatchBtn.addEventListener("click", async () => {
      ...
      try {
        const res = await fetch("/adobe/api/v1/tokens/credits/refresh-batch", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({}),
        });
  ...
  if (deleteTokensBatchBtn) {
    deleteTokensBatchBtn.addEventListener("click", async () => {
      ...
      try {
        const res = await fetch("/adobe/api/v1/tokens/delete-batch", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  if (exportTokensBtn) {
    exportTokensBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const res = await fetch("/adobe/api/v1/tokens/export", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
  ...
  if (exportCookiesBtn) {
    exportCookiesBtn.addEventListener("click", async () => {
      ...
      try {
        const selectedIds = Array.from(tokenSelectedIds);
        const payload = selectedIds.length ? { ids: selectedIds } : { ids: null };
        const res = await fetch("/adobe/api/v1/refresh-profiles/export-cookies", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
  ...
  async function loadConfig() {
    try {
      const res = await fetch("/adobe/api/v1/config");
      if (res.ok) {
        const data = await res.json();
  ...
  saveConfigBtn.addEventListener("click", async () => {
    ...
    try {
      // 保留未在此页面显示的配置项
      const currentRes = await fetch("/adobe/api/v1/config");
  ...
  async function importCookies() {
    ...
    try {
      ...
      const importOne = async (item) => {
        const res = await fetch("/adobe/api/v1/refresh-profiles/import-cookie", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function loadLogs() {
    ...
    try {
      ...
      const [runningResult, logsResult, statsResult] = await Promise.allSettled([
        fetch("/adobe/api/v1/logs/running?limit=200"),
        fetch(`/adobe/api/v1/logs?limit=${LOGS_PAGE_SIZE}&page=${logsCurrentPage}`),
        fetch(`/adobe/api/v1/logs/stats?range=${encodeURIComponent(rangeValue)}`),
      ]);
  ...
  async function openErrorDetailByCode(code) {
    ...
    try {
      const res = await fetch(`/adobe/api/v1/logs/errors/${encodeURIComponent(errCode)}`);
      if (!res.ok) {
        const txt = await res.text();
  ...
  if (clearLogsBtn) {
    clearLogsBtn.addEventListener("click", async () => {
      ...
      try {
        const res = await fetch("/adobe/api/v1/logs", { method: "DELETE" });
