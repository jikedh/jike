...
@app.middleware("http")
async def adobe_no_cache_static(request: Request, call_next):
    response = await call_next(request)
    path = str(request.url.path or "")
    if (
        path in {"/", "/login", "/adobe/", "/adobe/login"}
        or path.startswith("/static/")
        or path.startswith("/adobe/static/")
    ):
...
def _detect_image_mime(image_url: str, image_bytes: bytes, header_mime: str) -> str:
    header_mime = _normalize_image_mime(header_mime)
    if header_mime != "image/jpeg" or not image_bytes:
        return header_mime

    lower_url = str(image_url or "").split("?", 1)[0].lower()
...
def _load_input_images(messages) -> list[tuple[bytes, str]]:
    ...
    if not image_urls:
        logger.warning("adobe2api input image urls=0")
        return []

    loaded: list[tuple[bytes, str]] = []
    logger.warning("adobe2api input image urls=%s values=%s", len(image_urls), image_urls)
    for image_url in image_urls:
        if image_url.startswith("data:"):
            ...
            header_mime = (resp.headers.get("content-type") or "image/jpeg").split(
                ";"
            )[0].strip() or "image/jpeg"
            mime_type = _detect_image_mime(image_url, image_bytes, header_mime)
        ...
        logger.warning(
            "adobe2api loaded input image bytes=%s mime=%s",
            len(image_bytes),
            normalized_mime,
        )
        loaded.append((image_bytes, normalized_mime))
