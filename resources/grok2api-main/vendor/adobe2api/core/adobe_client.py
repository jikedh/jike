...
class AdobeClient:
    ...
    def generate(
        ...
        for candidate_index, payload in enumerate(
            self._build_payload_candidates(
                prompt=prompt,
                aspect_ratio=aspect_ratio,
                output_resolution=output_resolution,
                upstream_model_id=upstream_model_id,
            ...
            if (
                ...
                and source_image_ids
            ):
                logger.warning(
                    "gpt-image edit candidate=%s module=%s referenceBlobs=%s referenceImages=%s references=%s modelSpecificReferenceImages=%s",
                    candidate_index,
                    ...
                    (payload.get("modelSpecificPayload") or {}).get("referenceImages")
                    if isinstance(payload.get("modelSpecificPayload"), dict)
                    else None,
                )
            submit_resp = self._post_json(
                self.submit_url,
            ...
            if (
                ...
                logger.warning(
                    "gpt-image edit candidate=%s failed status=%s body=%s",
                    candidate_index,
