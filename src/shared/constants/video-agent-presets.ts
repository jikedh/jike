...
1. 我们要先提取全局角色档案，再创作分镜表。分镜表包含了镜号、场景、时长、角色、景别、画面、角度、运动、主体动作、信息点、声画关系、技参、转场、分镜图、分镜草图。
2. 要利用导演思维和编剧思维，构建我们内容中的每个剧情环节。
3. 剧情与剧情之间的连贯，要在场景改变后，照顾到物理空间的联想，不能不切实际地影响剧情。
4. 每一个切分都要严格识别到位。
5. 严禁按固定时长、平均时长、等间隔、每 N 秒、每 5 秒/10 秒/15 秒等机械规则切分分镜；分镜边界只能来自真实镜头切点、场景/空间变化、主体动作阶段变化、视角/景别/机位变化、关键叙事信息变化或声画关系变化。
6. 如果一个镜头持续时间较长但画面、主体动作、机位和叙事信息没有发生实质变化，必须保持为同一个分镜，不要为了“尽量细”而硬拆；如果短时间内发生多个真实镜头或动作阶段变化，必须按实际变化拆开，允许相邻分镜时长明显不均匀。
...
- “视频拉片分析”表列固定为：镜号 | 场景 | 时长 | 角色 | 景别 | 画面 | 角度 | 运动 | 主体动作 | 信息点 | 声画关系 | 技参 | 转场 | 分镜图 | 分镜草图。
...
- 分镜时长必须反映视频中真实镜头/动作/叙事变化，不能出现规律性的固定时长切段；若输出看起来像等分时间轴，视为错误结果。
