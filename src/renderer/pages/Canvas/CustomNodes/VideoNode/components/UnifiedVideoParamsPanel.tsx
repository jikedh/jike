...
const ButtonsControl = ({
  ...
  return (
    <div className="w-full">
      <div
        className={cn(twoRows ? "grid grid-cols-4 gap-2" : "flex w-full gap-2")}
      >
...
const ParamControl = ({
  ...
  if (param.controlType === "switch") {
    return <SwitchControl param={param} value={value} onChange={onChange} />;
  }
          ...
          {summary.length === 0 ? (
            ...
            <span className="flex items-center gap-1.5">
              {summary.map((item, index) => {
                const separator =
                  index === 0 ? null : (
                    <span key={`sep_${index}`} className="text-neutral-500">
                      |
                    </span>
                  );
        ...
        <div className="space-y-5">
          {paramConfig.params.map((param) => {
            ...
            return (
              <div
                key={param.key}
                className={isSwitch ? undefined : "space-y-2"}
              >
                {isSwitch ? null : (
