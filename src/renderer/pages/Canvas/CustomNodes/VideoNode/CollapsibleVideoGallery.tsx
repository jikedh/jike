...
import { IconRefresh, IconMenu } from "@tabler/icons-react";
...
export const CollapsibleVideoGallery = memo(({
  ...
  const handleRefreshVideo = useCallback(
    async (e: React.MouseEvent, index: number) => {
      ...
      if (!nodeId || !updateVideoNodeData || !videos[index]?.relativePath) {
        return;
      }
      ...
      try {
        ...
        // 创建 File 对象
        const ext = item.localFileName.split(".").pop() || "mp4";
  ...
  return (
    ...
    >
      <div className="relative h-full w-full overflow-hidden rounded-lg bg-background shadow-sm group">
        ...
        {/* 刷新按钮：仅在有本地文件时显示 */}
        {nodeId && updateVideoNodeData && videos[0]?.relativePath && (
          <button
            type="button"
            ...
            >
              {videos.map((item, index) => (
                ...
                >
                  ...
                  {/* 展开态中的刷新按钮 */}
                  {nodeId && updateVideoNodeData && item.relativePath && (
                    <button
                      type="button"
