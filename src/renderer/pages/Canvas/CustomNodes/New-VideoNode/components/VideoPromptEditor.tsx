...
import { AudioWaveformMentionPreview } from "./AudioWaveformPreview";
  ...
  (
    ...
    useEffect(() => {
      ...
      const getMediaMentionPill = (target: EventTarget | null) => {
        ...
        if (mentionType === "audio") {
          const audioUrl = mentionPill.dataset.url || mentionPill.dataset.fileUrl;
          if (!audioUrl) {
            return null;
          }
          return { element: mentionPill, type: "audio" as const, audioUrl };
        }
      ...
      const handleMouseOver = (event: MouseEvent) => {
        ...
        if (result.type === "audio") {
          setAudioPreview({
            url: result.audioUrl,
            label:
              result.element.dataset.mentionOriginalLabel || "音频参考",
    ...
    return (
      <>
        ...
        {audioPreview &&
          createPortal(
            <AudioWaveformMentionPreview
              audioUrl={audioPreview.url}
              label={audioPreview.label}
              position={{
                left: audioPreview.left,
                top: audioPreview.top,
              }}
            />,
            document.body,
          )}
