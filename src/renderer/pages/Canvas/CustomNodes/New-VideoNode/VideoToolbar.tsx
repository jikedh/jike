...
const extractTaskStatusInfo = (response: any) => {
    ...
    const progressRaw =
        nested?.progress ?? payload?.progress ?? output?.progress;
...
const VideoSubtitleRemovalPanel = ({
    ...
    const handleSend = useCallback(async () => {
        ...
        const rect: WuhenRect = {
            ...
            x2: Math.round(
                (cropRect.x - videoBounds.x + cropRect.width) * scaleX,
            ),
            y2: Math.round(
                (cropRect.y - videoBounds.y + cropRect.height) * scaleY,
            ),
        };
        ...
        <>
            ...
            <div className="nodrag nopan nowheel inline-flex h-10 items-center gap-1 overflow-hidden rounded-full border border-white/10 bg-[#2a2a2d] px-2 shadow-xl">
                {toolbarActions.map((item) => {
                    ...
                    const isActive =
                        item.key === "preview" ? isPreviewActive : false;
                    const isDisabled =
