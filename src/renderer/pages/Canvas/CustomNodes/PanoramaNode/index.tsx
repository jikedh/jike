  ...
  ({ id, data, selected, dragging }: NodeProps<PanoramaNodeType>) => {
      ...
      async (type: "single" | "4grid" | "12grid") => {
        ...
        if (type === "single") {
          ...
          if (type === "4grid") {
            for (let i = 0; i < 4; i++) {
              ...
              camera.lookAt(
                new THREE.Vector3(
                  Math.cos(0) * Math.sin(yaw),
                  Math.sin(0),
                  Math.cos(0) * Math.cos(yaw),
                ),
              );
              renderer.render(scene, camera);
              imagesData.push({
        ...
        try {
          const uploadedUrls = await Promise.all(
            imageUrls.map((item) =>
              uploadPanoramaScreenshot({ dataUrl: item, type }),
            ),
          );
        ...
        img.onerror = (error) => {
          ...
          imgNoCors.onload = () => {
            try {
              ...
              console.error(
                "PanoramaNode: Failed to create texture without CORS:",
                err,
              );
            }
          };
    ...
    useEffect(() => {
      ...
      if (fullscreenRendererRef.current) {
        ...
        if (
          fullscreenCanvasRef.current.contains(
            fullscreenRendererRef.current.domElement,
          )
        ) {
          fullscreenCanvasRef.current.removeChild(
            fullscreenRendererRef.current.domElement,
          );
        }
      }
      ...
      img.onload = () => {
        try {
          ...
          console.error(
            "PanoramaNode fullscreen: Failed to create texture:",
            error,
          );
        }
      };
      ...
      img.onerror = () => {
        ...
        imgNoCors.onload = () => {
          try {
            ...
            console.error(
              "PanoramaNode fullscreen: Failed to create texture without CORS:",
              err,
            );
          }
        };
