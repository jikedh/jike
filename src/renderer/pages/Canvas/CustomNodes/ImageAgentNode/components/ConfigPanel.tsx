/**
 * ============================================
 * ConfigPanel - 图片智能体节点配置面板组件
 * ============================================
 *
 * 【组件功能】
 * 当图片智能体节点被选中时，显示的配置面板。
 * 包含系统提示词编辑、AI 模型选择和生成按钮。
 *
 * 【布局结构】
 * ┌────────────────────────────────────────┐
 * │  ┌──────────────────────────────────┐  │
 * │  │     系统提示词编辑文本框           │  │
 * │  │     (min-height: 120px)          │  │
 * │  └──────────────────────────────────┘  │
 * │                                        │
 * │  ┌─────────────────┐  ┌──────────┐   │
 * │  │   模型下拉选择   │  │  生成按钮 │   │
 * │  └─────────────────┘  └──────────┘   │
 * └────────────────────────────────────────┘
 *
 * 【显示条件】
 * 仅当节点被选中（selected === true）时显示
 */
import { IconCheck, IconChevronDown, IconSend } from "@tabler/icons-react";
import { useState } from "react";
import { IMAGE_AGENT_MODELS } from "shared/constants/image-agent-presets";
import { cn } from "shared/utils/utils";
import { Button } from "@/components/ui/button";

/**
 * ============================================
 * ConfigPanel Props 接口定义
 * ============================================
 *
 * @property editableSystemPrompt - 当前可编辑的系统提示词内容
 * @property onSystemPromptChange - 系统提示词变化时的回调函数
 * @property currentModel - 当前选择的 AI 模型标识
 * @property onModelChange - 模型切换时的回调函数
 * @property isGenerating - 是否正在生成中（控制按钮状态）
 * @property onGenerate - 点击生成按钮的回调函数
 */
interface ConfigPanelProps {
  /** 当前可编辑的系统提示词内容 */
  editableSystemPrompt: string;

  /**
   * 系统提示词变化回调
   * @param value - 用户输入的新提示词内容
   */
  onSystemPromptChange: (value: string) => void;

  /** 当前选择的 AI 模型标识（如 qwen3.5-flash） */
  currentModel: string;

  /**
   * 模型切换回调
   * @param model - 用户选择的新模型标识
   */
  onModelChange: (model: string) => void;

  /** 是否正在生成中（生成中时按钮禁用） */
  isGenerating: boolean;

  /** 点击生成按钮的回调函数 */
  onGenerate: () => void;
}

/**
 * ============================================
 * ConfigPanel 组件
 * ============================================
 *
 * @param editableSystemPrompt - 系统提示词
 * @param onSystemPromptChange - 提示词变更回调
 * @param currentModel - 当前模型
 * @param onModelChange - 模型变更回调
 * @param isGenerating - 是否生成中
 * @param onGenerate - 生成回调
 */
export const ConfigPanel = ({
  editableSystemPrompt,
  onSystemPromptChange,
  currentModel,
  onModelChange,
  isGenerating,
  onGenerate,
}: ConfigPanelProps) => {
  // ============================================
  // 本地状态
  // ============================================

  /** 控制模型下拉菜单的显示/隐藏 */
  const [showModelDropdown, setShowModelDropdown] = useState(false);

  // ============================================
  // 渲染逻辑
  // ============================================

  return (
    <div className="nodrag nopan nowheel absolute left-1/2 -translate-x-1/2 top-[216px] w-[500px] rounded-2xl border border-white/[0.05] bg-[#1e1e20] p-3 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] animate-in fade-in slide-in-from-top-2 duration-200 z-30">
      {/* 系统提示词编辑区域 */}
      <div className="relative mb-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
        <textarea
          value={editableSystemPrompt}
          onChange={(e) => onSystemPromptChange(e.target.value)}
          placeholder="系统提示词..."
          className="nodrag nopan nowheel min-h-[120px] max-h-[300px] w-full overflow-y-auto bg-transparent text-sm leading-6 text-white/90 placeholder:text-white/30 resize-none outline-none"
        />
      </div>

      {/* 底部操作栏：模型选择 + 生成按钮 */}
      <div className="flex items-center justify-between gap-3">
        {/* ============================================
            模型下拉选择器
            ============================================ */}
        <div className="relative flex-1">
          <button
            onClick={() => setShowModelDropdown(!showModelDropdown)}
            className="flex w-full items-center justify-between rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-xs text-white/70 transition-colors hover:border-[#B43FEB]/30 hover:text-white/90 hover:bg-white/[0.04]"
          >
            {/* 显示当前选中的模型名称 */}
            <span>
              {IMAGE_AGENT_MODELS.find((m) => m.value === currentModel)
                ?.label || currentModel}
            </span>
            {/* 下拉箭头图标 */}
            <IconChevronDown
              size={14}
              className={cn(
                "transition-transform",
                showModelDropdown && "rotate-180",
              )}
            />
          </button>

          {/* 模型下拉选项列表 */}
          {showModelDropdown && (
            <div className="absolute left-0 right-0 top-full z-20 mt-1 rounded-lg border border-white/[0.06] bg-[#09090b] py-1 shadow-xl">
              {IMAGE_AGENT_MODELS.map((m) => (
                <button
                  key={m.value}
                  onClick={() => {
                    onModelChange(m.value);
                    setShowModelDropdown(false);
                  }}
                  className={cn(
                    "flex w-full items-center justify-between px-3 py-2 text-xs transition-colors",
                    currentModel === m.value
                      ? "bg-[#B43FEB]/20 text-[#B43FEB]"
                      : "text-white/60 hover:bg-white/5",
                  )}
                >
                  <span>{m.label}</span>
                  {/* 选中状态指示器 */}
                  {currentModel === m.value && <IconCheck size={14} />}
                </button>
              ))}
            </div>
          )}
        </div>

        {/* ============================================
            生成按钮
            ============================================ */}
        <Button
          disabled={isGenerating}
          onClick={onGenerate}
          className={cn(
            "gap-1.5 h-8 px-4 text-xs font-medium rounded-lg transition-colors active:scale-[0.97]",
            isGenerating
              ? "bg-white/10 text-white/40 cursor-not-allowed"
              : "bg-[#B43FEB] text-white hover:bg-[#B43FEB]/80",
          )}
        >
          <IconSend size={14} />
          {isGenerating ? "生成中..." : "生成"}
        </Button>
      </div>
    </div>
  );
};
