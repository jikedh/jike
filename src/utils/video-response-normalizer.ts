...
const standardCompletedStatusSet = new Set(["completed", "succeeded"]);
...
const extractStandardVideoItems = (response: any) => {
  const resultItems = normalizeResultItems(
  ...
  // 标准任务的兼容路径（按优先级兜底）
  const fallbackUrl =
    response?.metadata?.url || response?.data?.video_url || response?.data?.url;
  if (fallbackUrl) {
    return [
      {
        url: fallbackUrl,
...
export const normalizeVideoTaskResponse = (model: string, response: any) => {
  ...
  if (isSeedance20) {
    ...
    return {
      status:
        seedance20InProgressStatusMap[rawStatus] ??
  ...
  return {
    status:
      standardInProgressStatusMap[rawStatus] ?? GenerationStatus.IN_PROGRESS,
    progress,
    taskId,
    videoItems: [],
    missingResultUrl: false,
