...
export const ImageCropDialog = memo(({ open, imageUrl, onOpenChange, onConfirm }: ImageCropDialogProps) => {
    ...
    const applyAspectToRect = useCallback((rect: Rect, bounds: Rect) => {
        const minSize = 24
    ...
    const syncImageBounds = useCallback(() => {
        ...
        setCropRect((prev) => {
            if (!prev) {
                ...
                return cropRatio === 'free' ? baseRect : applyAspectToRect(baseRect, nextBounds)
            }

            const scaledRect = {
                x: (prev.x / (imageBounds?.width || prev.width || 1)) * nextBounds.width,
                y: (prev.y / (imageBounds?.height || prev.height || 1)) * nextBounds.height,
                width: (prev.width / (imageBounds?.width || prev.width || 1)) * nextBounds.width,
                height: (prev.height / (imageBounds?.height || prev.height || 1)) * nextBounds.height,
            }

            return cropRatio === 'free' ? scaledRect : applyAspectToRect(scaledRect, nextBounds)
        })
    ...
    const startDrag = useCallback((mode: DragMode, event: React.PointerEvent<HTMLDivElement>) => {
        if (!cropRect) {
            return
        }
    ...
    const handlePointerMove = useCallback((event: PointerEvent) => {
        ...
        if (mode === 'move') {
            ...
            nextRect.y = clamp(start.y + dy, 0, imageBounds.height - start.height)
            setCropRect(nextRect)
            return
        }

        if (mode.includes('e')) {
            nextRect.width = clamp(start.width + dx, minSize, imageBounds.width - start.x)
        }

        if (mode.includes('s')) {
            nextRect.height = clamp(start.height + dy, minSize, imageBounds.height - start.y)
        }
            ...
            <DialogContent className="w-[min(980px,96vw)] max-h-[92vh] overflow-hidden bg-slate-50 p-0">
                ...
                <div className="space-y-4 px-5 py-4">
                    <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_220px]">
                        <div ref={viewportRef} className="relative min-h-130 overflow-hidden rounded-xl bg-slate-900">
                            {imageUrl ? (
                                <>
                                    <img
                                        ref={imageRef}
                        ...
                        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4">
                            ...
                            {cropRatio === 'free' ? (
                                <div className="space-y-2">
                                    <div className="text-sm font-medium text-slate-700">自由比例（宽 : 高）</div>
                                    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
                                        <Input
                                            ...
                                            onChange={(event) => {
                                                ...
                                                setFreeRatioWidth(Number.isFinite(nextValue) && nextValue > 0 ? Math.round(nextValue) : 1)
                                            }}
                                        />
                                        ...
                                        <Input
                                            ...
                                            onChange={(event) => {
                                                const nextValue = Number(event.target.value)
                                                setFreeRatioHeight(Number.isFinite(nextValue) && nextValue > 0 ? Math.round(nextValue) : 1)
                                            }}
                                        />
                                    </div>
                                </div>
