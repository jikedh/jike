...
type MinimaxHailuo23ParamsPanelProps = {
    ...
    // 更新 metadata
    onMetadataChange: (metadata: MinimaxHailuo23ParamsPanelProps['metadata']) => void
}
...
export const MinimaxHailuo23ParamsPanel = ({
    ...
    // 更新 metadata 中指定字段
    const updateMetadata = (updates: Partial<NonNullable<MinimaxHailuo23ParamsPanelProps['metadata']>>) => {
        onMetadataChange({
            ...metadata,
            ...updates,
        })
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        {RESOLUTION_OPTIONS.map((item) => {
                                            // 1080p 仅支持 6 秒时长
                                            const isDisabled = item.value === '1080p' && (duration ?? 6) !== 6
                                            const isActive = (metadata?.resolution ?? '768p') === item.value
                                            return (
                                                <button
                                                    key={item.value}
                                                    type="button"
                                                    onClick={() => !isDisabled && updateMetadata({ resolution: item.value })}
                                                    disabled={isDisabled}
                                                    className={cn(
