import { NodeToolbar, Position, type NodeProps, useStore, useReactFlow } from '@xyflow/react'
import { memo, useCallback, useMemo, useRef, useState, useEffect } from 'react'
import { IconMusic, IconUpload, IconPlayerPlay, IconPlayerPause, IconTrash, IconDownload, IconCut, IconX, IconCheck } from '@tabler/icons-react'
...
import type { AudioNodeType } from '@/types/flow'
  ...
  if (status === GenerationStatus.FAILED) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center gap-3">
        ...
        <div className="text-sm text-red-400">{data.error?.message || '生成失败'}</div>
      </div>
    )
  }
      ...
      <div className="flex items-center gap-4 w-full">
        ...
        <div className="flex-1 flex flex-col gap-1.5">
                ...
                <div
                  ...
                  onMouseDown={(e) => {
                    ...
                    const handleMove = (moveE: MouseEvent) => {
                      ...
                      const delta = ((moveE.clientX - startX) / rect.width) * duration
                      const newStart = Math.max(0, Math.min(trimEnd - 0.1, startValue + delta))
                ...
                <div
                  ...
                  onMouseDown={(e) => {
                    ...
                    const handleMove = (moveE: MouseEvent) => {
                      const rect = (e.target as HTMLElement).parentElement!.getBoundingClientRect()
                      const delta = ((moveE.clientX - startX) / rect.width) * duration
                      const newEnd = Math.max(trimStart + 0.1, Math.min(duration, startValue + delta))
          ...
          <div className="flex justify-between text-xs text-white/50">
            <span>{formatTime(isTrimming ? Math.max(trimStart, Math.min(trimEnd, currentTime)) : currentTime)}</span>
        ...
        <div className="w-full flex flex-col gap-3 p-3 rounded-lg bg-white/5 border border-white/10">
          <div className="flex items-center justify-between text-xs">
            ...
            <div className={cn(
              ...
              isValidTrim ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"
            )}>
              <span>时长:</span>
...
export const AudioNode = memo(({
  ...
  return (
    ...
    >
      <div className="group/node relative" style={{ pointerEvents: isTrimming ? 'none' : 'auto' }}>
        <ButtonHandle
          type="target"
