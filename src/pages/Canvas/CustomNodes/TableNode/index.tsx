...
import type { TableNodeType, CharacterTableRow } from '@/types/flow'

const TABLE_COLUMNS = ['姓名', '基础设定', '性格特征', '核心动机', '核心关系', '习惯和兴趣']
...
const EditableCell = memo(({ value, rowIndex, column, onUpdate, maxWidth, className }: EditableCellProps) => {
  ...
  return (
    <div
      className={cn('whitespace-pre-wrap break-words leading-relaxed cursor-text w-full h-full', className)}
...
const TableBody = memo(({ rows, columns, onUpdateCell, maxWidth, cellClassName }: TableBodyProps) => {
  if (!rows || rows.length === 0) {
    return (
      <tr>
        <td colSpan={columns.length} className={cn('px-3 py-8 text-center text-[#8D8D8E]/50 text-xs', cellClassName)}>
          暂无数据
        </td>
      </tr>
    )
      ...
      {rows.map((row, rowIndex) => (
        <tr key={rowIndex} className="hover:bg-white/[0.03] transition-colors">
          {columns.map((col) => (
            <td
              key={col}
              className={cn('relative px-3 py-2 border-b border-r border-white/[0.06] text-[#8D8D8E] text-xs align-top', cellClassName)}
            >
              <EditableCell
...
export const TableNode = memo(({ id, data, selected, width, height, dragging }: NodeProps<TableNodeType>) => {
  const duplicateNode = useCanvasFlowStore((state) => state.duplicateNode)
  const deleteNode = useCanvasFlowStore((state) => state.deleteNode)
  const updateTableNodeData = useCanvasFlowStore((state) => state.updateTableNodeData)
  ...
  const handleUpdateCell = useCallback((rowIndex: number, column: string, value: string) => {
    const newRows = [...(rows || [])]
    newRows[rowIndex] = {
      ...newRows[rowIndex],
      [column]: value,
  ...
  const fullscreenContent = (
    <div className="fixed inset-0 z-[9999] bg-gradient-to-br from-[#141418] to-[#0d0d10] flex flex-col">
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/10">
        <div className="text-base font-semibold text-white">{title || '角色设计表'}</div>
