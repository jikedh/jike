...
pub async fn download_image_to_file(
    ...
    domain::image_to_file(&url, &file_path)
        .await
        .map(|r| serde_json::json!({ "success": true, "data": r }))
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
pub async fn download_image_with_save_dialog(
    url: String,
    default_name: String,
    app: tauri::AppHandle,
) -> Result<serde_json::Value, String> {
        ...
        .as_deref()
    {
        ...
        Some("mp3") => dialog.add_filter("MP3 音频", &["mp3"]),
        Some("wav") => dialog.add_filter("WAV 音频", &["wav"]),
        Some("ogg") => dialog.add_filter("OGG 音频", &["ogg"]),
        Some("aac") => dialog.add_filter("AAC 音频", &["aac"]),
    ...
    match target {
        ...
        Some(p) => domain::image_to_file(&url, &p)
            .await
            .map(|r| serde_json::json!({ "success": true, "data": r }))
            .map_err(|e| e.to_string()),
    }
