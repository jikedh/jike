...
#[tauri::command]
pub async fn storage_ensure_project(
    base: String,
    project: String,
) -> Result<serde_json::Value, String> {
    domain::ensure_project_handler(&base, &project)
...
pub async fn storage_list_projects(base: String) -> Result<serde_json::Value, String> {
    domain::list_projects_handler(&base)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_save_canvas(
    base: String,
    project: String,
    data: serde_json::Value,
) -> Result<serde_json::Value, String> {
    domain::save_canvas_handler(&base, &project, data)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_load_canvas(
    base: String,
    project: String,
) -> Result<serde_json::Value, String> {
    domain::load_canvas_handler(&base, &project)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_save_media(
    base: String,
    relative_path: String,
    buffer: Vec<u8>,
) -> Result<serde_json::Value, String> {
    domain::save_media_handler(&base, &relative_path, buffer)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_read_media(
    base: String,
    relative_path: String,
) -> Result<serde_json::Value, String> {
    domain::read_media_handler(&base, &relative_path)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_write_raw_file(
    base: String,
    relative_path: String,
    buffer: Vec<u8>,
) -> Result<serde_json::Value, String> {
    domain::write_raw_file_handler(&base, &relative_path, buffer)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_read_raw_file(
    base: String,
    relative_path: String,
) -> Result<serde_json::Value, String> {
    domain::read_raw_file_handler(&base, &relative_path)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}
...
pub async fn storage_scan_asset_library(base: String) -> Result<serde_json::Value, String> {
    domain::scan_asset_library_handler(&base)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_delete_raw_path(
    base: String,
    relative_path: String,
) -> Result<serde_json::Value, String> {
    domain::delete_raw_path_handler(&base, &relative_path)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_rename_raw_path(
    base: String,
    old_path: String,
    new_path: String,
) -> Result<serde_json::Value, String> {
    domain::rename_raw_path_handler(&base, &old_path, &new_path)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_list_media(
    base: String,
    project: String,
    media_type: String,
) -> Result<serde_json::Value, String> {
    domain::list_media_handler(&base, &project, &media_type)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_delete_media(
    base: String,
    relative_path: String,
) -> Result<serde_json::Value, String> {
    domain::delete_media_handler(&base, &relative_path)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_download_media(
    base: String,
    url: String,
    relative_path: String,
) -> Result<serde_json::Value, String> {
    domain::download_media_handler(&base, &url, &relative_path)
        .await
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
pub async fn storage_save_buffer_to_file(
    default_name: String,
    buffer: Vec<u8>,
    app: tauri::AppHandle,
) -> Result<serde_json::Value, String> {
    ...
    dialog.save_file(move |path| {
        let s = path.and_then(|p| p.into_path().ok()).map(|pb| {
    ...
    match target {
        ...
        Some(p) => domain::write_bytes_to_path(&p, &buffer)
            .map(|r| serde_json::to_value(r).unwrap())
            .map_err(|e| e.to_string()),
    }
}
...
pub async fn storage_rename_project(
    ...
    domain::rename_project_handler(&base, &old_name, &new_name)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_delete_project(
    base: String,
    project: String,
) -> Result<serde_json::Value, String> {
    domain::delete_project_handler(&base, &project)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_copy_project(
    base: String,
    src: String,
    dest: String,
) -> Result<serde_json::Value, String> {
    domain::copy_project_handler(&base, &src, &dest)
        .map(|r| serde_json::to_value(r).unwrap())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn storage_export_project(
    base: String,
    project: String,
    app: tauri::AppHandle,
) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn storage_import_project(
    base: String,
    app: tauri::AppHandle,
) -> Result<serde_json::Value, String> {
    use tauri_plugin_dialog::DialogExt;
...
#[tauri::command]
pub async fn storage_ensure_project_dir(
    base: String,
    project: String,
) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn storage_write_file(
    base: String,
    relative_path: String,
    buffer: Vec<u8>,
) -> Result<serde_json::Value, String> {
